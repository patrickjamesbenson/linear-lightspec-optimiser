...
if uploaded_file:
    ...
    # === EXTRACT LUMINAIRE NAME + OPTIC DESC ===
    luminaire_info_line = next((line for line in parsed['header'] if line.startswith("[LUMINAIRE]")), "[LUMINAIRE] Not Found")
    luminaire_name_full = luminaire_info_line.replace("[LUMINAIRE]", "").strip()

    # Parse luminaire details
    luminaire_parts = luminaire_name_full.split('-')
    base_name = luminaire_parts[0].strip() if len(luminaire_parts) > 0 else "Unknown"
    optic_description = luminaire_parts[1].strip() if len(luminaire_parts) > 1 else "Unknown"
    ...
    with st.expander("📂 Base File Summary (IES Metadata + Photometric Parameters)", expanded=True):
        ...
        metadata_dict = {
            ...
            "Luminaire Description": luminaire_info_line,
    ...
    with st.expander("📂 Base Build Methodology", expanded=True):
        # If no lengths, allow unlocking/editing
    ...
    if st.session_state['lengths_list']:
        ...
        for length in st.session_state['lengths_list']:
            ...
            luminaire_file_name = f"{base_name}_{optic_description}_{length:.3f}m_{tier}"
    ...
    if st.session_state['lengths_list']:
        ...
        for length in st.session_state['lengths_list']:
            ...
            filename = f"{base_name}_{optic_description}_{length:.3f}m.ies"
