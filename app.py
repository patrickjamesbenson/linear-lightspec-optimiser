...
if 'locked' not in st.session_state:
    ...
    st.session_state['lmw_increment'] = 115
...
if uploaded_file:
    ...
    luminaire_description = luminaire_info.replace("[LUMINAIRE]", "").strip()

    # === PARSE COMPONENTS ===
    # Example description: "BLine 8585D 11.6W - Diffused Down - 80CRI - 3000K"
    parts = luminaire_description.split('-')
    model_name = parts[0].strip() if len(parts) > 0 else "Unknown Model"
    optic_desc = parts[1].strip() if len(parts) > 1 else "Unknown Optic"
    cri_value = parts[2].strip() if len(parts) > 2 else "N/A"
    cct_value = parts[3].strip() if len(parts) > 3 else "N/A"
    ...
    with st.expander("📂 Base Build Methodology", expanded=False):
        if st.session_state['lengths_list']:
            st.info("🔒 Base Build locked. End Plate: {:.1f} mm | LED Pitch: {:.1f} mm".format(
    ...
    with st.expander("🔒 System lm/W Efficiency Increment", expanded=False):
        ...
        **115 lm/W is the standard deviation in lumen output between ECG driver current increments in our current range.**
        ...
        This field is editable and should only be changed by advanced users.
        """)
        st.session_state['lmw_increment'] = st.number_input("System lm/W Efficiency Increment", min_value=50, max_value=300,
                                                             value=st.session_state['lmw_increment'], step=1)
    ...
    if st.session_state['lengths_list']:
        ...
        for length in st.session_state['lengths_list']:
            ...
            luminaire_file_name = f"{model_name} {optic_desc}_{length:.3f}m_{tier}"
        ...
        for idx, row in enumerate(table_rows):
            ...
            for col, val in zip(row_cols[1:], row.values()):
                col.write(val)

        export_df = pd.DataFrame(table_rows).drop(columns=["Delete"])
        st.download_button("Download CSV Summary", data=export_df.to_csv(index=False).encode('utf-8'), file_name="Selected_Lengths_Summary.csv")
    ...
    with st.expander("Design Optimisation", expanded=True):
        target_lux = st.number_input("Target Lux Level", min_value=0, value=400)
        achieved_lux = st.number_input("Achieved Lux Level", min_value=0, value=700)

        if achieved_lux and target_lux:
            diff_percent = round(100 * (achieved_lux - target_lux) / target_lux, 1)
            diff_abs = abs(diff_percent)

            st.metric("Difference", f"{diff_percent}%")
            current_lm_m = new_lm_per_m

            required_change_lm_m = round(current_lm_m * (diff_percent / 100), 1)
            increments_needed = int(abs(required_change_lm_m) / st.session_state['lmw_increment']) + (1 if required_change_lm_m > 0 else 0)

            if diff_percent > 0:
                st.warning(f"⚠️ Consider reducing by {increments_needed} increments or dimming to match target lux.")
            else:
                st.success(f"🟢 Consider increasing by {increments_needed} increments or increasing output to match target lux.")
    ...
    if st.session_state['lengths_list']:
        ...
        for length in st.session_state['lengths_list']:
            ...
            filename = f"{model_name} {optic_desc}_{length:.3f}m.ies"
            new_file = create_ies_file(updated_header, scaled_data)
            files_to_zip[filename] = new_file

        zip_buffer = create_zip(files_to_zip)

        st.download_button("Generate IES Files & Download ZIP", data=zip_buffer, file_name="Optimised_IES_Files.zip")
